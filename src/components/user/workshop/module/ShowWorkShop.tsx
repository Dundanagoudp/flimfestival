    ...
    <main className="w-full px-4 bg-white">
      <div className="px-10 py-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {workshops.map((workshop) => (
          <Reveal key={workshop._id} delay={0.1} y={-10} transition={{ type: 'spring', stiffness: 90, damping: 18, mass: 0.8 }}>
          <Card key={workshop._id} className="overflow-hidden shadow-md rounded-2xl hover:scale-102 transition-all duration-300">
            ...
            <CardContent className="p-4">
              ...
              <Link href={workshop?.registrationFormUrl} target="_blank" rel="noopener noreferrer">
                <Button className="w-full rounded-full bg-primary text-black hover:bg-yellow-300 hover:scale-102 transition-all duration-300">
                  Register
                </Button>
              </Link>
            </CardContent>
