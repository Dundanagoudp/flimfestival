...
// PDF — plan and day (one per plan / one per day; re-upload overrides)
...
export async function getPlanPdf(planId: string): Promise<string | null> {
  try {
    const response = await apiClient.get<PdfApiResponse>(
      `${BASE}/${planId}/pdf`
    )
    const res = response.data
    if (res.success && res.data?.pdfUrl) return res.data.pdfUrl
    return null
  } catch (err: any) {
...
export async function uploadPlanPdf(
  ...
  try {
    const response = await apiClient.post<PdfApiResponse>(
      `${BASE}/${planId}/pdf`,
      form
    )
    const res = response.data
    ...
    const msg =
      err?.response?.data?.error ||
      err?.message ||
      "Failed to upload plan PDF"
    throw new Error(msg)
  }
}

export async function getDayPdf(
  planId: string,
  dayId: string
): Promise<string | null> {
  try {
    const response = await apiClient.get<PdfApiResponse>(
      `${BASE}/${planId}/days/${dayId}/pdf`
    )
    const res = response.data
    if (res.success && res.data?.pdfUrl) return res.data.pdfUrl
    return null
    ...
    const msg =
      err?.response?.data?.error ||
      err?.message ||
      "Failed to get day PDF"
    throw new Error(msg)
  }
}

export async function uploadDayPdf(
  planId: string,
  dayId: string,
  file: File
): Promise<string> {
